...
confirmBtn.addEventListener("click", () => {
    ...
    setTimeout(() => {
        ...
        if (total >= PROFILE_THRESHOLD && window.getChapter() === 3) {
            ...
            setTimeout(() => {
                afterAnswer.textContent += " a file that wasn't there before just surfaced on the parent network.";
                const link = document.createElement("a");
                link.href = "voicemail.html";
                link.textContent = "→ recovered audio, internal extension 4471";
                link.style.display = "block";
                link.style.marginTop = "10px";
                link.style.color = "#ff6a3d";
                afterAnswer.appendChild(link);
            }, 1200);
